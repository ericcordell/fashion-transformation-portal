...
function daysUntilTarget(targetDate) {
  ...
  const quarterMap = {
    'Q1': new Date('2026-04-30'),  // End of Q1
    'Q2': new Date('2026-07-31'),  // End of Q2
    'Q3': new Date('2026-10-31'),  // End of Q3
    'Q4': new Date('2027-01-31'),  // End of Q4
  };
  
  // Check if it's a quarter reference
  for (const [q, date] of Object.entries(quarterMap)) {
    if (targetDate.includes(q)) {
      ...
      <tr onclick="openCardModal('${card.id}')">
        <td class="wpr-program-cell">
          <div class="wpr-program-name">
            ...
            ${card.critical ? '<span class="wpr-critical-badge">⭐ CRITICAL</span>' : ''}
          </div>
        </td>
        <td>
