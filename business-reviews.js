...
const BIG_ROCK_PROGRAM_MAP = {
  rock1: { // Trend Anticipation
    q1: ['OPIF-329924', 'OPIF-329920'],  // Strategy Hub, Forecast Service
    q2: ['OPIF-327330', 'OPIF-329925', 'OPIF-325568'],  // Design Hub, Shared Event Layer, AP Tool Line Plan
    q3: ['OPIF-329923', 'OPIF-327331'],  // Shared Merch Strategy, Centric Visual Board
    q4: ['OPIF-327330', 'OPIF-325568'],  // Design Hub Phase 2, AP Tool integration
  },
  rock2: { // Proactive Allocation
    q1: ['OPIF-324932', 'OPIF-324925'],  // AEX Stability Q1, Automated Item Setup
    q2: ['OPIF-324932', 'OPIF-327329'],  // AEX Stability Q2, Fashion Fixture Allocation
    q3: ['OPIF-327726', 'OPIF-327333', 'OPIF-325568'],  // Enterprise Wave Planning, Tagging Pilot, AP Tool
    q4: ['OPIF-327334', 'OPIF-327726'],  // Tag-Based Recommendations, Advanced Allocation
  },
  rock3: { // Connected Systems
    q1: ['OPIF-324932', 'OPIF-324926'],  // AEX Stability, Shared Item Repository
    q2: ['OPIF-324925', 'OPIF-324928', 'OPIF-329922'],  // Automated Item Setup, AI Repository, BQ Enterprise
    q3: ['OPIF-327327', 'OPIF-327328'],  // BAM/Collab Integration, Automated Size/Pack
    q4: ['OPIF-324925', 'OPIF-327327'],  // Full convergence, API integrations
  },
};
...
function getBigRockInitiatives(rockId, quarter) {
  const opifs = BIG_ROCK_PROGRAM_MAP[rockId]?.[`q${quarter}`] || [];
  const allCards = window.PILLARS ? window.PILLARS.flatMap(p => p.cards || []) : [];
  
  return opifs.map(opif => {
    const card = allCards.find(c => c.opif === opif);
    if (!card) return null;
...
function renderBigRockQuarter(rockId, quarter, quarterLabel) {
  ...
  const initiativesList = initiatives.map(init => `
    <div class="bigrock-initiative" onclick="openCardModal('${init.id}')">
      ...
      <span class="bigrock-init-opif">${init.opif}</span>
    </div>
  `).join('');
